...
impl<'a, 'source> AstUniversalTransform<'a, 'source> {
    ...
    pub(crate) fn prepend_helpers(&mut self, program: &mut Program<'a>) {
        ...
        if self.uses_ref {
            statements.push(self.import_named("use", &self.helper_local("_$use")));
        }
        ...
        match &attr.value {
            Some(JSXAttributeValue::ExpressionContainer(container)) => {
                ...
                if let Some(directive) = key.strip_prefix("use:") {
                    let value = jsx_expression_to_expression(&container.expression, self.allocator);
                    let statement =
                        self.universal_use_statement(attr.span, element_id, directive, value);
                    ref_exprs.insert(0, statement);
                    return Ok(());
                }
                if key == "children" {
    ...
    fn universal_use_statement(
        ...
        self.uses_ref = true;
        let accessor = arrow_return_expression(self.allocator, span, value);
        let call = self.call_identifier(
            span,
            &self.helper_local("_$use"),
            vec![
                expression_to_argument(self.identifier_expression(span, directive)),
                self.identifier_arg(span, element_id),
                expression_to_argument(accessor),
            ],
        );
        self.expression_statement(span, call)
    }
    ...
    fn add_static_attr(
        ...
        value: Expression<'a>,
        _has_spread: bool,
        _init_props: &mut std::vec::Vec<ObjectPropertyKind<'a>>,
        attr_exprs: &mut std::vec::Vec<Statement<'a>>,
    ) {
        // Solid 1.x universal renderers initialize attributes through
        // `setProp`; passing a static props object to `createElement` is a
            ...
            match attr {
                ...
                JSXAttributeItem::Attribute(attr) => {
                    ...
                    let can_native_spread = key != "ref"
                        && !key.split_once(':').is_some_and(|(namespace, _)| {
                            matches!(
                                namespace,
    ...
    fn universal_ref_statements(
        ...
        let ref_call = |ctx: &mut Self, target: Expression<'a>| {
            ...
            ctx.call_identifier(
                span,
                &ctx.helper_local("_$use"),
                vec![
                    expression_to_argument(target),
    ...
    fn wrap_dynamics(
        ...
        if dynamics.len() == 1 {
            ...
            let effect = self.arrow_with_param_names(
                span,
                vec!["_$p"],
                self.ast()
                    .vec1(self.ast().statement_return(span, Some(set_prop))),
            );
            return Some(self.expression_statement(
                span,
                self.call_identifier(span, &effect_local, vec![expression_to_argument(effect)]),
            ));
        }
        ...
        for (index, slot) in dynamics.into_iter().enumerate() {
            ...
            declarations.push(self.variable_statement(slot_span, &value_name, slot.value));
            ...
            let prev_member = Expression::StaticMemberExpression(
            ...
            let set_prop = self.set_prop_expression(
                ...
                &slot.key,
                value_ident,
                Some(prev_member.clone_in(self.allocator)),
            );
            let assignment = self.assign_static_member(slot_span, "_p$", &prop_name, set_prop);
            updates.push(self.expression_statement(
                slot_span,
        ...
        let effect = self.arrow_with_param_names(span, vec!["_p$"], declarations);
        ...
        Some(self.expression_statement(
            ...
            self.call_identifier(
                ...
                &effect_local,
                vec![
                    expression_to_argument(effect),
                    expression_to_argument(initial),
                ],
            ),
        ))
    }

    fn assign_static_member(
        &self,
        span: Span,
        object: &str,
        name: &str,
    ...
    fn universal_component_ref(
        ...
        let ref_call = |ctx: &mut Self, ref_identifier: Expression<'a>| {
            ctx.call_expression(
                span,
                ref_identifier,
                vec![ctx.identifier_expression(span, "r$")],
            )
        };
        ...
        if let Expression::CallExpression(_) = &value {
            ...
            let target = self.identifier_expression(span, &ref_id);
            let apply = ref_call(self, target);
    ...
    pub(crate) fn process_statements(&mut self, statements: &mut ArenaVec<'a, Statement<'a>>) {
        ...
        let mut body = ArenaVec::new_in(self.allocator);
        let queued: std::vec::Vec<Statement<'a>> = statements
            .drain(..)
            .flat_map(|statement| {
                crate::shared::statements::split_multi_declarator_jsx(self.allocator, statement)
            })
            .collect();
        for mut statement in queued {
    ...
    fn lower_variable_jsx_initializer(
        ...
        let exported = matches!(statement, Statement::ExportNamedDeclaration(_));
        ...
        for index in 0..declaration.declarations.len() {
            ...
            let init = if matches!(
                ...
                let (mut init, lowered) = self.lower_statement_position_jsx(init);
                if let Some(lowered) = lowered {
                    if self.function_parent_stack.is_empty() && self.pending_this_capture.is_some()
                    {
                        let span = init.span();
                        init = self.setup_iife(span, lowered, init);
                        init = crate::shared::transform::finalize_root_capture(self, span, init);
                        setup.get_or_insert_with(std::vec::Vec::new);
                    } else if exported {
                        init = self.setup_iife(init.span(), lowered, init);
                        setup.get_or_insert_with(std::vec::Vec::new);
                    } else {
    ...
    fn lower_statement_position_jsx(
        ...
        match expression {
            Expression::JSXElement(element) => {
                ...
                match lowered {
                    Ok((replacement, setup)) => (replacement, Some(setup)),
                    Err(error) => {
                        self.error = Some(error.to_string());
            ...
            Expression::JSXFragment(fragment) => {
                ...
                match lowered {
                    Ok(replacement) => (replacement, Some(std::vec::Vec::new())),
                    Err(error) => {
                        self.error = Some(error.to_string());
                        (
                            Expression::JSXFragment(fragment),
