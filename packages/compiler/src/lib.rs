...
#[cfg_attr(not(feature = "native-facts"), napi)]
...
/// Analyze original-source JSX with the same Solid 1.x classification
/// authority used by lowering and emit a total execution contract.
pub fn analyze_execution_contract(code: &str, options: &TransformOptions) -> Result<String> {
    let source_type = source_type_for_filename(options.filename.as_deref())?;
    let allocator = Allocator::default();
    let parsed = Parser::new(&allocator, code, source_type)
        .with_options(ParseOptions {
    ...
    facts::execution_contract(code, &parsed.program, options).map_err(Error::from_reason)
}
