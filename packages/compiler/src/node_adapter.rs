//! The Node-API adapter over the reusable compiler core.
//!
//! Everything Node-shaped lives here: the nullable transport option structs,
//! the `#[napi]` exports, and the established validation ordering the npm
//! package's callers depend on. The core knows nothing about any of it.
...
pub use napi::Either as CompilerOption;
...
pub fn transform(code: String, options: Option<TransformOptions>) -> Result<TransformResult> {
    ...
    let output = crate::compiler::compile_for_node_adapter(&code, &options).map_err(host_error)?;
    Ok(TransformResult {
        code: output.code,
...
fn host_error(error: crate::CompileError) -> Error {
...
fn core_options(options: TransformOptions) -> Result<CompileOptions> {
    ...
    Ok(CompileOptions {
        ...
        // The Node addon compiles; execution facts are a Rust-consumer
        // concern and have no transport representation here.
...
/// ordering on the exceptional paths that cannot be represented by the typed
/// Rust options: a parse failure outranks both, and a `requireImportSource`
/// skip outranks the option validation that would otherwise reject the call.
fn legacy_preflight(
    code: &str,
    options: &TransformOptions,
    validation_error: &'static str,
) -> Result<TransformResult> {
    let source_type = crate::compiler::source_type_for_filename(options.filename.as_deref())
        .map_err(host_error)?;
...
mod tests {
    ...
    fn require_import_source_skip_precedes_module_name_validation() {
        ...
        assert!(error.to_string().contains(MISSING_MODULE_NAME));
    }
    ...
    fn explicit_empty_module_name_remains_accepted() {
        ...
        .expect("Solid 1.x accepts an explicitly empty moduleName");
    }
}
