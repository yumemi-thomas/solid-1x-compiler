...
use crate::shared::constants::{
    child_properties, inline_elements, reserved_namespace, ALWAYS_CLOSE_ELEMENTS, BLOCK_ELEMENTS,
};
...
impl<'a> AstDomTransform<'a, '_> {
    ...
    pub(crate) fn plan_attributes(
        ...
        tag_name: &str,
        nested_native_child: bool,
    ) -> Result<AttrPlanOutcome<'a>> {
        // Solid 1.x's child transform evaluates a nested literal `style`
        // after the no-inline wrapper was installed and consequently drops
        // it. Template roots (including roots inside a fragment) retain the
        // runtime `style()` call. Preserve that observable 1.x behavior.
        let filtered;
        let attributes = if !self.inline_styles && nested_native_child {
            filtered = attributes
                .iter()
                .filter(|item| !is_literal_style_attribute(item))
                .map(|item| item.clone_in(self.allocator))
    ...
    pub(crate) fn lower_template_attributes(
        ...
        element_id: &str,
        has_children: bool,
        promoted_children_span: Option<Span>,
        template: &mut String,
        ...
        for plan in plans {
            ...
            // A value that never reaches the runtime — dropped, or frozen into
            // the template string.
            if matches!(
                disposition,
            ...
            match disposition {
                ...
                PlanDisposition::Inline(value) => match value {
                    None => {
                        let key = template_attribute_name(&plan.key, tag_name);
                        append_bare_attribute(template, &key, self.omit_attribute_spacing)
                    }
                    Some(value) => {
                        let key = template_attribute_name(&plan.key, tag_name);
                        self.append_static_attribute_value(template, &key, &value)
                    }
                },
        ...
        if has_spread {
            exprs.push(self.spread_attribute_statement(
                attributes,
                element_id,
                tag_name,
                has_children,
                promoted_children_span.is_some(),
            )?);
        }
    ...
    fn classify_plan(&self, plan: &AttrPlan<'a>) -> PlanDisposition {
        ...
        if plan.key.starts_with("bool:") {
            return match &plan.value {
                PlanValue::None => PlanDisposition::Skip,
                ...
                PlanValue::Expr(Expression::StringLiteral(value)) => {
                    if !value.value.is_empty() && value.value != "0" {
                        PlanDisposition::Inline(None)
                    } else {
                        PlanDisposition::Skip
                    }
                }
                PlanValue::Expr(Expression::NumericLiteral(value)) => {
                    if value.value != 0.0 {
                        PlanDisposition::Inline(None)
                    } else {
                        PlanDisposition::Skip
                ...
                PlanValue::Expr(Expression::BooleanLiteral(value)) => {
                    if value.value {
                        PlanDisposition::Inline(None)
                    } else {
                        PlanDisposition::Skip
                ...
                PlanValue::Expr(Expression::NullLiteral(_)) => PlanDisposition::Skip,
                PlanValue::Expr(Expression::Identifier(identifier))
                    if identifier.name == "undefined" =>
                {
                    PlanDisposition::Skip
                }
                PlanValue::Expr(_) => PlanDisposition::Runtime,
            };
        }
        if plan.key.starts_with("attr:") {
            return match &plan.value {
                PlanValue::Literal(value) => PlanDisposition::Inline(Some(value.clone())),
                PlanValue::Expr(Expression::StringLiteral(value)) => {
                    PlanDisposition::Inline(Some(value.value.to_string()))
                }
                PlanValue::Expr(Expression::NumericLiteral(value)) => {
                    PlanDisposition::Inline(Some(format_number(value.value)))
                }
                PlanValue::None => PlanDisposition::Skip,
                PlanValue::Expr(_) => PlanDisposition::Runtime,
            };
        }
        let reserved = plan.style_property
            || plan.class_property
            || plan
                .key
                .split_once(':')
                .is_some_and(|(prefix, rest)| !rest.is_empty() && reserved_namespace(prefix));
        match &plan.value {
            PlanValue::None => {
                if plan.key.starts_with("use:") {
                    PlanDisposition::Runtime
                } else if reserved {
            ...
            PlanValue::Expr(expression) => {
                ...
                match expression {
                    // Babel only evaluates string and numeric containers into
                    // template attributes. Boolean containers stay in the
                    // runtime branch (`checked={true}` is a property write).
                    Expression::BooleanLiteral(_) => PlanDisposition::Runtime,
    ...
    fn lower_runtime_attribute(
        ...
        let raw = match plan.value {
            ...
            PlanValue::None if plan.key.starts_with("use:") => {
        ...
        {
            // Promoted values are reported by child insertion; a value the
            // promotion skipped (a spread is present, or real children exist)
            // is dropped here.
            // The promoted value is reported by child insertion instead. What
            // is left here was dropped: real children shadowed it, a later
            // duplicate won, or a spread kept it out of the spread object.
            {
                    ...
                    .filter(|span| Some(**span) != promoted_children_span)
                {
                    for kind in [
                        ...
                        if self.semantic_trace.has_site(*span, kind) {
                            self.semantic_trace.value(
                                *span,
                                kind,
                                crate::semantic_trace::ValueDecision::Elided,
        ...
        if let Some(directive) = plan.key.strip_prefix("use:") {
            self.template_state.uses_ref = true;
            // The value becomes an accessor handed to the directive, which
            // decides when (and whether) to read it.
            for span in &semantic_spans {
                self.semantic_trace.value(
                    *span,
                    crate::semantic_trace::ExecutionSiteKind::NativeAttribute,
            ...
            let value = self.arrow_with_return(span, std::vec::Vec::new(), raw);
            let call = self.call_identifier(
                span,
                "_$use",
                vec![
                    self.identifier_expression(span, directive),
                    self.identifier_expression(span, element_id),
                    value,
                ],
            );
            front_groups.push(vec![self.ast().statement_expression(span, call)]);
            return Ok(());
        }

        if let Some(event) = plan.key.strip_prefix("on:") {
            ...
            let call = self.call_identifier(
                ...
                vec![
                    self.identifier_expression(span, element_id),
                    self.ast()
                        .expression_string_literal(span, self.ast().atom(event), None),
                    raw,
                ],
            );
            front_groups.push(vec![self.ast().statement_expression(span, call)]);
        ...
        if let Some(event) = plan.key.strip_prefix("oncapture:") {
            for span in &semantic_spans {
                self.semantic_trace.callback(
                    *span,
                    crate::semantic_trace::ExecutionSiteKind::EventHandler,
            ...
            exprs.push(self.ast().statement_expression(span, call));
            return Ok(());
        }
        ...
        if dynamic {
            let class_property = plan.class_property || plan.key.starts_with("class:");
    ...
    fn dom_ref_statements(
            ...
            )
        {
            ...
            let call = self.call_identifier(
                span,
                "_$use",
                vec![value, self.identifier_expression(span, element_id)],
            );
    ...
    pub(crate) fn try_append_planned_static_attributes(
        ...
        let AttrPlanOutcome {
            ...
            elided_value_spans,
            children_replacement,
        } = self.plan_attributes(attributes, tag_name, nested_native_child)?;
        ...
        for plan in &plans {
            match self.classify_plan(plan) {
                PlanDisposition::Skip => {}
                PlanDisposition::Inline(value) => {
                    pending.push((template_attribute_name(&plan.key, tag_name), value))
                }
                // Nothing was appended and no decision was recorded, so the
                // caller falls back to the full emission path.
...
fn inline_namespace_name(key: &str) -> &str {
    key.strip_prefix("bool:")
        .or_else(|| key.strip_prefix("attr:"))
        .unwrap_or(key)
}

fn template_attribute_name(key: &str, tag_name: &str) -> String {
    let key = inline_namespace_name(key);
...
fn is_literal_style_attribute(item: &JSXAttributeItem<'_>) -> bool {
    ...
    match &attribute.value {
        ...
        Some(oxc_ast::ast::JSXAttributeValue::ExpressionContainer(container)) => container
            .expression
            .as_expression()
