...
pub(crate) struct DomTemplate {
    ...
    /// Solid 1.x template mode: 1 = importNode, 2 = SVG, 3 = MathML.
...
impl<'a> AstDomTransform<'a, '_> {
    pub(crate) fn prepend_helpers(&mut self, program: &mut Program<'a>) -> Result<()> {
        ...
        if self.template_state.uses_ref {
            statements.push(self.import_named("use", "_$use"));
        }
        ...
        if self.template_state.uses_class_list {
            statements.push(self.import_named("classList", "_$classList"));
        }
        ...
        if self.template_state.uses_set_bool_attribute {
            statements.push(self.import_named("setBoolAttribute", "_$setBoolAttribute"));
        }
        ...
        if self.template_state.uses_add_event_listener {
            statements.push(self.import_named("addEventListener", "_$addEventListener"));
        }
    ...
    fn template_declaration(&self, template: &DomTemplate) -> Statement<'a> {
        ...
        if let Some(flag) = template.flag {
            for value in match flag {
                1 => [true, false, false],
                2 => [false, true, false],
                3 => [false, false, true],
                _ => unreachable!("known template mode"),
            } {
                args.push(self.ast().expression_boolean_literal(span, value));
            }
        }
