...
use crate::shared::utils::{
    decode_html_entities, dedupe_attributes, format_number, is_literal_only_expression, StaticValue,
};
...
pub(crate) struct AttrPlan<'a> {
    /// Span stamped on generated AST nodes, and therefore on source maps.
    pub(crate) span: Span,
    /// Original value spans, used only by semantic reporting. Keeping these
    /// separate from `span` means tracing can never move a source location.
    /// A plan carries more than one when several source attributes were
    /// combined into it (Babel's `class`/`className` merge).
    pub(crate) semantic_spans: std::vec::Vec<Span>,
...
pub(crate) struct AttrPlanOutcome<'a> {
    ...
    /// Literal-only leaves have no observable execution to report.
...
impl<'a> AttrPlanner<'a, '_> {
    ...
    pub(crate) fn plan_attributes(
        ...
        // Object leaves that the census counts as their own sites, so folding
        // one into a static string can report exactly that leaf as elided.
        ...
        for attr in dedupe_attributes(attributes) {
            ...
            if key == "_hk" || (key == "data-hk" && !self.is_ssr) {
                continue;
            }
            ...
            plans.push(AttrPlan {
                span: attr.span,
                semantic_spans: semantic_span.into_iter().collect(),
                key,
                value,
        ...
        let children_replacement = if self.is_ssr {
            self.special_case_stateful_plans(tag_name, &mut plans)
        } else {
            None
        };
        if self.is_ssr {
            self.merge_ssr_namespace_objects(&mut plans, "style", "style");
            if plans.iter().any(|plan| plan.key == "classList") {
                self.normalize_ssr_class_namespace_duplicates(&mut plans);
            } else {
                self.merge_ssr_namespace_objects(&mut plans, "class", "classList");
            }
        } else {
    ...
    fn normalize_ssr_class_namespace_duplicates(&self, plans: &mut std::vec::Vec<AttrPlan<'a>>) {
        let class_namespaces: std::vec::Vec<usize> = plans
            .iter()
            .enumerate()
            .filter_map(|(index, plan)| plan.key.starts_with("class:").then_some(index))
            .collect();
        if class_namespaces.is_empty() {
            return;
        }
        let keep = *class_namespaces.last().expect("non-empty");
        let removals: std::vec::Vec<usize> = plans
            .iter()
            .enumerate()
            .filter_map(|(index, plan)| {
                (index != keep && (plan.key.starts_with("class:") || plan.key == "classList"))
                    .then_some(index)
            })
            .collect();
    ...
    /// SSR's `normalizeAttributes` folds `class:*` and `style:*` attributes
    /// into the corresponding object-valued classList/style attribute before
    /// class and style serialization.
    fn merge_ssr_namespace_objects(
        &self,
        ...
        let selected: std::vec::Vec<usize> = plans
            .iter()
            .enumerate()
            .filter_map(|(index, plan)| plan.key.starts_with(&prefix).then_some(index))
            .collect();
        ...
        for index in &selected {
            ...
            let name = plan.key.trim_start_matches(&prefix);
            let value = match &plan.value {
                PlanValue::Expr(value) => value.clone_in(self.allocator),
                PlanValue::Literal(value) => {
                    self.ast()
                        .expression_string_literal(plan.span, self.ast().atom(value), None)
                }
                PlanValue::None => self.ast().expression_boolean_literal(plan.span, true),
            };
            properties.push(object_property(self.allocator, plan.span, name, value));
        }

        if let Some(existing) = plans.iter_mut().find(|plan| plan.key == target) {
            if let PlanValue::Expr(Expression::ObjectExpression(object)) = &mut existing.value {
        ...
        let marker_static = plans[first].marker_static;
        for index in selected.iter().skip(1).rev() {
            plans.remove(*index);
        }
        plans[first] = AttrPlan {
            span,
            semantic_spans: std::vec::Vec::new(),
            key: target.to_string(),
            value: PlanValue::Expr(
                self.ast()
    ...
    fn arrow_with_return(&self, span: Span, value: Expression<'a>) -> Expression<'a> {
        let statements = self
            .ast()
            .vec1(self.ast().statement_expression(span, value));
    ...
    fn split_style_object(&self, plans: &mut std::vec::Vec<AttrPlan<'a>>) {
        ...
        for property in object.properties.iter() {
            ...
            split_plans.push(AttrPlan {
                span: property.span,
                semantic_spans: vec![property.value.span()],
    ...
    fn split_class_array(&self, plans: &mut std::vec::Vec<AttrPlan<'a>>) {
        ...
        plans.insert(
            ...
            AttrPlan {
                span,
                semantic_spans: vec![object_span],
                key: "class".to_string(),
    ...
    fn combine_class_attributes(&self, plans: &mut std::vec::Vec<AttrPlan<'a>>) {
        let class_indices: std::vec::Vec<usize> = plans
            .iter()
            .enumerate()
            .filter(|(_, plan)| plan.key == "class" || plan.key == "className")
            .map(|(index, _)| index)
            .collect();
        ...
        let merged_spans = class_indices
            .iter()
            .flat_map(|index| plans[*index].semantic_spans.clone())
        ...
        plans[first].semantic_spans = merged_spans;
