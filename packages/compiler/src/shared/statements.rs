...
impl<'a> AstDomTransform<'a, '_> {
    ...
    pub(crate) fn process_statements(&mut self, statements: &mut ArenaVec<'a, Statement<'a>>) {
        ...
        let queued: std::vec::Vec<Statement<'a>> = statements
            .drain(..)
            .flat_map(|statement| split_multi_declarator_jsx(self.allocator, statement))
            .collect();
        for mut statement in queued {
            ...
            if let Some(mut setup) = self.lower_variable_jsx_initializer(&mut statement) {
                ...
                crate::shared::transform::lower_deferred_jsx_statements(self, &mut setup);
                body.extend(setup);
            ...
            if let Some(mut setup) = self.lower_return_jsx(&mut statement) {
                self.lower_deferred_statement_jsx(&mut statement);
                if self.in_class_method_scope() {
                    if let Some(capture) = self.take_this_capture_statement(statement.span()) {
                        body.push(capture);
        ...
        // Lower any JSX left in ordinary (non-root) statements.
                ...
                Expression::JSXElement(element) => {
                    match self.lower_root_element_with_setup(&element) {
                        Ok((replacement, statements)) => {
                            if self.function_parent_stack.is_empty()
                                && self.pending_this_capture.is_some()
                            {
                                setup.get_or_insert_with(std::vec::Vec::new);
                                let replacement =
                                    self.setup_iife(element.span, statements, replacement);
...
pub(crate) fn split_multi_declarator_jsx<'a>(
    ...
    {
        return vec![Statement::VariableDeclaration(declaration)];
    }
