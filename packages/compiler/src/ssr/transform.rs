...
impl<'a, 'source> AstSsrTransform<'a, 'source> {
    ...
    pub(crate) fn repair_identifier_arrow_body(
        &self,
        arrow: &mut oxc_ast::ast::ArrowFunctionExpression<'a>,
    ) {
        if !arrow.expression {
        ...
        let raw = &self.source[arrow.span.start as usize..arrow.span.end as usize];
        let body = raw
            .split_once("=>")
            .map(|(_, body)| body.trim())
            .unwrap_or_default();
        if !crate::shared::utils::is_valid_babel_identifier(body) {
            return;
        }
        let span = arrow.span;
        arrow.body.statements.clear();
        arrow.body.statements.push(
            self.ast().statement_expression(
                span,
                self.ast()
                    .expression_identifier(span, self.ast().ident(body)),
            ),
        );
    }
    ...
    pub(crate) fn prepend_helpers(&mut self, program: &mut Program<'a>) {
        ...
        if self.uses_ssr_class_name {
            statements.push(self.import_named("ssrClassList", "_$ssrClassList"));
        }
    ...
    fn lower_element_impl(
        ...
        if self.hydratable && element_name(&element.opening_element.name)? == "head" {
            return self.lower_no_hydration_head(element, true);
        }
        if element
            .opening_element
            .attributes
        ...
        let template = self.ssr_template(element, top_level)?;
        self.finalize_template(element, template)
    }

    fn lower_no_hydration_head(
        &mut self,
        element: &JSXElement<'a>,
        retain_child_markers: bool,
    ) -> Result<Expression<'a>> {
        let head = if retain_child_markers {
            let template = self.ssr_template(element, false)?;
            self.finalize_template(element, template)?
        } else {
            let previous = self.hydratable;
            self.hydratable = false;
            let head = self.lower_element_impl(element, false);
            self.hydratable = previous;
            head?
        };
        if !self
            .built_in_imports
        ...
        {
            self.built_in_imports.push("NoHydration".to_string());
        }
        self.uses_create_component = true;
        let children = self.object_getter_property(element.span, "children", head);
        let props = self
            .ast()
            .expression_object(element.span, self.ast().vec1(children));
        Ok(self.call_expression(
            element.span,
            self.ast()
    ...
    fn finalize_template(
        ...
        if self.hydratable
            && wont_escape
            && template.parts.len() == 2
            && template.values.len() == 1
            && template.declarations.iter().all(Option::is_none)
            && template.post_declarations.is_empty()
            && matches!(
                ...
                Some(Expression::CallExpression(call))
                    if matches!(
                        ...
                        Expression::Identifier(identifier)
                            if identifier.name == "_$ssrHydrationKey"
                    )
            )
        {
            let key = template.values.pop().expect("single hydration-key value");
            let template_member = |index: f64| {
            ...
            return Ok(self.ast().expression_binary(
                span,
                prefix,
                oxc_ast::ast::BinaryOperator::Addition,
                template_member(1.0),
    ...
    fn lower_component(&mut self, element: &JSXElement<'a>) -> Result<Expression<'a>> {
        ...
        for (attribute_index, attr) in element.opening_element.attributes.iter().enumerate() {
            if let Some((first, selected, value, dynamic)) = &component_class {
                if selected.contains(&attribute_index) {
                    if attribute_index == *first {
            ...
            if name == "ref" {
                if let Some(JSXAttributeValue::ExpressionContainer(container)) = &attr.value {
                    if container.expression.as_expression().is_some() {
                        // Solid 1.x SSR does not forward component refs.
                        continue;
                    }
                }
            }
    ...
    fn component_class_normalization(
        &mut self,
        attributes: &[JSXAttributeItem<'a>],
        span: Span,
    ) -> Option<(usize, std::vec::Vec<usize>, Expression<'a>, bool)> {
        let selected: std::vec::Vec<usize> = attributes
            .iter()
            .enumerate()
            .filter_map(|(index, attribute)| {
                ...
                matches!(name.name.as_str(), "class" | "className" | "classList").then_some(index)
            })
            .collect();
        if selected.len() < 2 {
            return None;
        ...
        for (position, index) in selected.iter().enumerate() {
            let is_last = position + 1 == selected.len();
            let JSXAttributeItem::Attribute(attribute) = &attributes[*index] else {
                unreachable!()
            };
            let name = match &attribute.name {
                oxc_ast::ast::JSXAttributeName::Identifier(name) => name.name.as_str(),
                _ => unreachable!(),
            };
            match &attribute.value {
                Some(JSXAttributeValue::StringLiteral(value)) => {
                    quasis
                        .last_mut()
                        .expect("class quasi")
                        .push_str(&decode_html_entities(&value.value));
                    if !is_last {
                        quasis.last_mut().expect("class quasi").push(' ');
                    }
                }
                Some(JSXAttributeValue::ExpressionContainer(container)) => {
                    ...
                    if name == "classList" {
                        if let Expression::ObjectExpression(object) = expression {
                            if !object.properties.iter().any(|property| {
                                ...
                                dynamic |= self.classify().is_dynamic(
                    ...
                    values.push(self.ast().expression_logical(
                        container.span,
        ...
        let value = self.template_literal_from_parts(span, quasis, values);
        Some((selected[0], selected, value, dynamic))
    }
        ...
        for child in &filtered {
            match child {
                ...
                JSXChild::ExpressionContainer(container) => {
                    ...
                    // SSR never runs transformCondition for component
                    // children; the raw conditional is returned by the
                    // generated getter.
                    ...
                    let value = if wrap {
                        // SSR's createTemplate does not apply the client
                        // memo wrapper. For multiple component children Babel
                        // then strips the accessor down to its body.
                        inner
                    } else {
    ...
    fn spread_props(
        ...
        let class_normalization = self.component_class_normalization(attributes, Span::new(0, 0));
        for (attribute_index, attr) in attributes.iter().enumerate() {
            if let Some((first, selected, value, dynamic)) = &class_normalization {
                if selected.contains(&attribute_index) {
                    if attribute_index == *first {
                        running_props.push(component_property(
                            self,
    ...
    fn spread_children_expression(
        ...
        for child in children {
            match child {
                ...
                JSXChild::Spread(spread) => {
                    let expression = spread.expression.clone_in(self.allocator);
                    // SSR treats spread children as already spread values;
                    // unlike ordinary expression containers they are not
                    // accessor-wrapped before escaping.
    ...
    fn ssr_template(
        ...
        self.normalize_ssr_class_attributes(&mut outcome.plans);
    ...
    /// SSR `normalizeAttributes`: class, className, and classList attributes
    /// collapse into the first class attribute. Fixed classList objects are
    /// folded directly into the surrounding template literal.
    fn normalize_ssr_class_attributes(&mut self, plans: &mut std::vec::Vec<AttrPlan<'a>>) {
        let indices: std::vec::Vec<usize> = plans
            .iter()
            .enumerate()
            .filter_map(|(index, plan)| {
                matches!(plan.key.as_str(), "class" | "className" | "classList").then_some(index)
            })
            .collect();
        if indices.len() <= 1 {
            return;
        ...
        for (position, index) in indices.iter().enumerate() {
            ...
            match &plan.value {
                PlanValue::Literal(text) => {
                    quasis.last_mut().expect("class quasi").push_str(text);
                    if !is_last {
                        quasis.last_mut().expect("class quasi").push(' ');
                    }
                }
                ...
                PlanValue::Expr(expression) => {
                    ...
                    if plan.key == "classList" {
                        ...
                        value = self.helper_call(plan.span, "_$ssrClassList", vec![value]);
                    }
                    values.push(
        ...
        let first = indices[0];
        plans[first].key = "class".to_string();
        plans[first].class_property = true;
        plans[first].value = if values.is_empty() {
            PlanValue::Expr(self.ast().expression_string_literal(
                plans[first].span,
                self.ast().atom(&quasis[0]),
                None,
            ))
        } else if values.len() == 1 && quasis[0].is_empty() && quasis[1].is_empty() {
            PlanValue::Expr(values.pop().expect("single class value"))
        } else {
            PlanValue::Expr(self.template_literal_from_parts(plans[first].span, quasis, values))
        };
    ...
    fn append_planned_attribute(
        ...
        let mut key = plan.key;
        let class_property = plan.class_property;
        let span = plan.span;
        let reserved = plan.class_property
            || key
                .split_once(':')
                .is_some_and(|(prefix, _)| reserved_namespace(prefix));
        let output_key = ssr_attribute_name(tag_name, &key);
        ...
        let expression = match plan.value {
            PlanValue::None => {
                ...
                template.current_mut().push_str(&format!(" {output_key}"));
                return Ok(());
            }
            PlanValue::Literal(text) => {
                if reserved || child_properties(&key) {
                    ...
                    if boolean_attributes(&output_key) {
                        template.current_mut().push_str(&format!(" {output_key}"));
                        return Ok(());
                    }
                    let text = normalize_static_attribute_value(&output_key, &text);
                    append_ssr_static_attribute(template.current_mut(), &output_key, &text);
                    return Ok(());
                }
            }
            ...
            match expression {
                ...
                Expression::BooleanLiteral(literal) => {
                    if boolean_attributes(&output_key) {
                        ...
                        append_ssr_static_attribute(
                            template.current_mut(),
                            &output_key,
                ...
                Expression::StringLiteral(literal) => {
                    if boolean_attributes(&output_key) {
                        ...
                        let text = normalize_static_attribute_value(&output_key, &literal.value);
                        append_ssr_static_attribute(template.current_mut(), &output_key, &text);
                    }
                }
        ...
        if key == "ref" {
            // Solid 1.x SSR drops native refs completely.
            return Ok(());
        }
        if key.starts_with("use:") || key.starts_with("prop:") || key.starts_with("on") {
            return Ok(());
        }
        ...
        if let Some(name) = key
            .strip_prefix("bool:")
            .or_else(|| boolean_attributes(&key).then_some(output_key.as_str()))
        {
        ...
        let was_class_list = key == "classList";
        ...
        if key == "class" && class_property && !was_class_list {
        ...
        if !do_escape || is_literal_expression(&value) {
    ...
    fn set_attr(
        ...
        let mut name = match key.split_once(':') {
        ...
        name = match name.as_str() {
            ...
            _ if !crate::shared::constants::svg_elements(tag_name) => name.to_ascii_lowercase(),
            _ => name,
        };
        self.uses_ssr_attribute = true;
        ...
        let _ = is_dynamic;
        if template.current_mut().is_empty() {
            if let Some(previous) = template.values.pop() {
                template.values.push(self.ast().expression_binary(
                    span,
                    previous,
                    oxc_ast::ast::BinaryOperator::Addition,
                    attr,
    ...
    fn ssr_class_value(&mut self, span: Span, value: Expression<'a>) -> Expression<'a> {
        ...
        self.uses_ssr_class_name = true;
        self.helper_call(span, "_$ssrClassList", vec![value])
    }
    ...
    fn transform_classlist_object(
        ...
        for (index, property) in object.properties.iter().enumerate() {
            ...
            let static_truthiness = match &property.value {
                Expression::BooleanLiteral(literal) => Some(literal.value),
                Expression::Identifier(identifier) => {
                    self.bindings.static_bool(identifier.name.as_str())
                }
                _ => None,
            };
            if let Some(truthy) = static_truthiness {
                if !truthy {
                    continue;
                }
                match static_key {
    ...
    fn append_children(
        ...
        for child in children {
            match child {
                ...
                JSXChild::Element(child_element) => {
                    if element_name(&child_element.opening_element.name)? == "head" {
                        let wrapper = self.lower_no_hydration_head(child_element, false)?;
    ...
    fn dynamic_child_value(
        ...
        if matches!(expression, Expression::ArrowFunctionExpression(_)) {
            ...
            let body = raw
                ...
                .unwrap_or_default();
            if crate::shared::utils::is_valid_babel_identifier(body) {
                return self.arrow_return_expression(
                    span,
                    self.ast()
                        .expression_identifier(span, self.ast().ident(body)),
                );
    ...
    fn escape_expression_in_place(
        ...
        match value {
            ...
            Expression::ArrowFunctionExpression(arrow) => {
                // Oxc's cloned JSX expression-body representation can lose
                // the mutable body expression while still printing it from
                // the source span. Reconstruct the simple identifier case
                // before applying Babel's return-position escaping.
                let raw = &self.source[arrow.span.start as usize..arrow.span.end as usize];
                let source_body = raw
                    .split_once("=>")
                    .map(|(_, body)| body.trim())
                    .unwrap_or_default();
                // In an immediately-invoked expression Oxc can assign the
                // arrow the enclosing call's span (`... => value)()`), even
                // though its body AST is the bare identifier. Strip only
                // that exact enclosure before applying the identifier fast
                // path; do not mistake an actual `value()` body for one.
                let source_identifier =
                    if crate::shared::utils::is_valid_babel_identifier(source_body) {
                        Some(source_body)
                    } else {
                        source_body
                            .strip_suffix(")()")
                            .map(str::trim)
                            .filter(|body| crate::shared::utils::is_valid_babel_identifier(body))
                    };
                if let Some(source_body) = source_identifier {
                    let span = arrow.span;
                    let identifier = self
                        .ast()
                        .expression_identifier(span, self.ast().ident(source_body));
                    let escaped = if attr {
                        self.escape_attribute_expression(span, identifier)
                    } else {
                        self.escape_expression(span, identifier)
                    };
                    if arrow.params.items.is_empty() {
                        *value = self.arrow_return_expression(span, escaped);
                        return;
                    }
                    ...
                    arrow
                        .body
                        .statements
                        .push(self.ast().statement_expression(span, escaped));
                    return;
                }
                ...
                if expression_body && statements.len() == 1 {
                    if let Statement::ExpressionStatement(statement) = &mut statements[0] {
                        ...
                        self.escape_expression_in_place(&mut expression, attr, escape_literals);
                        statement.expression = expression;
                    } else {
                        // Oxc may retain an expression-bodied arrow as a
                        // synthetic return statement while `expression`
                        // remains true.
            ...
            Expression::CallExpression(call) if is_function_callee(&call.callee) => {
                // Babel does not retain parentheses in its AST. Recurse into
                // the function callee (through any Oxc parenthesis nodes) and
                // escape each return position.
                let callee_span = call.callee.span();
                let mut callee = std::mem::replace(
                    &mut call.callee,
                    self.ast().expression_null_literal(callee_span),
                );
                self.escape_expression_in_place(&mut callee, attr, escape_literals);
                call.callee = callee;
                // Escaping the IIFE result is equivalent to rewriting each
                // return and avoids relying on expression-bodied-arrow
                // statement storage during AST mutation.
                if let Expression::ArrowFunctionExpression(arrow) = &mut call.callee {
                    if arrow.expression {
                        let raw = &self.source[arrow.span.start as usize..arrow.span.end as usize];
                        let body = raw
                            .split_once("=>")
                        ...
                        if crate::shared::utils::is_valid_babel_identifier(body) {
                            ...
                            arrow.body.statements.push(
                                self.ast().statement_expression(
                                    arrow_span,
                                    self.ast()
                                        .expression_identifier(arrow_span, self.ast().ident(body)),
                                ),
                            );
                        } else if arrow.body.statements.is_empty() {
                            arrow.expression = false;
                        }
                    }
                }
            ...
            match statement {
                Statement::ReturnStatement(ret) => {
                    if let Some(argument) = ret.argument.as_mut() {
                        ...
                        let mut value =
                            std::mem::replace(argument, self.ast().expression_null_literal(span));
                        self.escape_expression_in_place(&mut value, attr, escape_literals);
                        *argument = value;
                    }
                }
...
impl<'a> crate::shared::mode_lower::ModeLower<'a> for AstSsrTransform<'a, '_> {
    ...
    fn memo_wrap_dynamic_child(&mut self, span: Span, thunk: Expression<'a>) -> Expression<'a> {
        ...
        // other dynamic expressions are represented by an arrow.
        if matches!(thunk, Expression::ArrowFunctionExpression(_)) {
            self.unwrap_expression_arrow(thunk)
        } else {
            self.call_expression(span, thunk, std::vec::Vec::new())
        }
    }
}

fn is_function_callee(expression: &Expression<'_>) -> bool {
    match expression {
        Expression::ArrowFunctionExpression(_) | Expression::FunctionExpression(_) => true,
        Expression::ParenthesizedExpression(parenthesized) => {
            is_function_callee(&parenthesized.expression)
        }
        _ => false,
    }
}

fn ssr_attribute_name(tag_name: &str, name: &str) -> String {
    let name = match name {
        "className" => "class",
        "htmlFor" => "for",
        _ => name,
