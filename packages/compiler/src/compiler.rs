//! The reusable compiler core.
//!
//! Everything a Rust consumer needs is here, expressed in owned Rust types:
//! no Oxc allocator, AST node, or Node-API value crosses this interface. The
//! Node addon is one adapter over this core (see `node_adapter`), not the
//! other way round.
...
/// A wrapper import setting without any Node-API representation in its
/// interface. Babel spells this as an import-name string with falsy disabling
/// the wrap; `Default` selects the mode's usual name.
...
pub struct CompileOptions {
    ...
    /// Unsupported output modes return a configuration error.
...
pub struct CompileOutput {
    ...
    /// Present only when [`CompileOptions::semantic_trace`] is enabled. The
    /// decisions are recorded by the lowering that produced `code`, then
    /// reconciled against an independent census of the source, so they
    /// describe this exact output rather than a re-derivation of it.
...
fn compile_inner(source: &str, options: &CompileOptions) -> Result<CompileOutput, CompileError> {
    if options.semantic_trace && options.generate != Generate::Dom {
        return Err(CompileError::configuration(
            "Solid 1 execution facts currently support the DOM output mode only",
        ));
    }
    let allocator = Allocator::default();
    let Some(mut program) = parse(&allocator, source, options)? else {
    ...
    match options.generate {
        Generate::Dom => {
            ...
            semantic_trace = std::mem::take(&mut transform.semantic_trace)
                .finish()
...
/// Parses the source, honoring Babel's `requireImportSource` preprocess:
/// `Ok(None)` means the file has no matching `@jsxImportSource` pragma and
/// must pass through untouched.
fn parse<'a>(
    allocator: &'a Allocator,
    source: &'a str,
    options: &CompileOptions,
) -> Result<Option<oxc_ast::ast::Program<'a>>, CompileError> {
...
fn dom_transform_config(options: &CompileOptions, built_ins: Vec<String>) -> DomTransformConfig {
    DomTransformConfig {
        ...
        // Solid 1.x accepts `dev` as an inert option. Hydration walk
        // validation (`getFirstChild`/`getNextSibling`) was added later and
...
mod tests {
    ...
    /// The decisions a source produces, as `(source text, decision name)`.
    fn decisions(source: &str) -> Vec<(&str, &'static str)> {
        let trace = compile(
            source,
            &CompileOptions {
                semantic_trace: true,
        ...
        trace
            ...
            .iter()
            .map(|site| {
                let text = &source[site.span.start as usize..site.span.end as usize];
                let decision = match site.decision {
                    crate::TerminalDecision::Value(decision) => decision.name(),
                    crate::TerminalDecision::Callback(decision) => decision.name(),
                };
                (text, decision)
            })
            .collect()
    ...
    /// Totality is proven rather than assumed: `finish()` would have failed
    /// the compile above on any unresolved or conflicting site, so a returned
    /// trace already means every censused site was decided by the lowering
    /// that ran.
    #[test]
    fn every_censused_site_is_decided() {
        let source = r#"
...
"#;
    ...
    // Babel's `hasChildren` gate: a `children` attribute is discarded whenever
    // the element has JSX children. The trace reports the discarded expression
    // as elided because the lowering that dropped it said so — there is no
    // second copy of the rule to fall out of step.
    #[test]
    fn shadowed_children_attribute_is_elided() {
        let decisions = decisions("const view = <Comp children={other()}><div>D</div></Comp>;");
        assert!(
            decisions.contains(&("other()", "elided")),
            "expected the shadowed children attribute to be elided, got {decisions:?}"
        );
    }

    #[test]
    fn unshadowed_children_attribute_still_executes() {
        let decisions = decisions("const view = <Comp children={other()} />;");
        assert!(
            decisions.contains(&("other()", "caller-context")),
            "expected the children attribute to stay live, got {decisions:?}"
        );
    }

    // A literal leaf (`color: "red"`) is never censused — it has no runtime
    // execution to report. A resolvable binding does, and folding it into the
    // template string is what `elided` means.
    #[test]
    fn a_static_attribute_folded_into_the_template_is_elided() {
        let decisions =
            decisions("const shade = \"red\";\nconst view = <div style={{ color: shade }} />;");
        assert!(
            decisions.contains(&("shade", "elided")),
            "expected the inlined style value to be elided, got {decisions:?}"
        );
    }
    ...
    fn an_event_handler_is_a_deferred_callback() {
        ...
        assert!(
            decisions.contains(&("() => act()", "later-event")),
            "expected the handler to be a later-event callback, got {decisions:?}"
        );
    }

    #[test]
    fn tracing_leaves_generated_code_untouched() {
        let source = "const view = <div id={id()}>{count()}</div>;";
        let plain = compile(source, &CompileOptions::default()).expect("compile");
        let traced = compile(
            source,
            &CompileOptions {
                semantic_trace: true,
