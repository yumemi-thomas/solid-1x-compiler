// Solid 1.x Babel vs Oxc output parity ratchet.
//
// Every fixture from babel-plugin-jsx-dom-expressions on dom-expressions#main
// is compiled with BOTH compilers under identical options
...
// Compiler failures and invalid generated JavaScript are meaningful
// divergences too. Keep them in the ratchet instead of aborting the suite, as
// the total-semantic-trace cross-mode harness does.
function fixtureDiff(mode, fixture) {
  const source = readFixtureSource(mode, fixture);
  return compareSource(source, fixture, modes[mode].options);
}
  ...
  for (const mode of Object.keys(modes)) {
    describe(mode, () => {
      it.each(fixtureNames(mode))("%s", fixture => {
        const diff = fixtureDiff(mode, fixture);
        if (update) {
