// Adversarial Solid 1.x Babel-vs-Oxc parity probes.
//
...
// Known differences are ratcheted independently so every improvement or
// regression is visible without losing the rest of this high-cardinality
// corpus.
...
`,
  ...
  // --- Solid 1.x legacy surface: namespace directives, the @once marker,
  // ref/event ordering, property-vs-attribute routing, and spread boundaries.
  // These are intentionally granular: a compiler fix should delete a small,
  // named expectation rather than rewriting a monolithic fixture diff. ------
...
const expectedDir = path.join(__dirname, "parity", "expected-probes");
const update = process.env.UPDATE_PARITY === "1";
const report = process.env.PARITY_REPORT === "1";
const reportRows = [];
const referenceRejected = new Set([
  "dom/1x empty reserved namespace values",
  "dom-hydratable/1x empty reserved namespace values",
  "dom-no-inline-styles/1x empty reserved namespace values",
  "dom-wrapperless/1x empty reserved namespace values",
  "ssr/1x empty reserved namespace values",
  "ssr-hydratable/1x empty reserved namespace values",
  "dynamic/1x empty reserved namespace values",
...
const caseEntries = Object.entries(cases).map(([name, source]) => ({
  name,
  source,
  id: caseId(name)
}));
...
for (const entry of caseEntries) {
  if (ids.has(entry.id)) throw new Error(`duplicate parity probe id: ${entry.id}`);
  ids.add(entry.id);
}
...
describe("Solid 1.x Babel vs Oxc parity probes", () => {
    ...
    describe(mode, () => {
      test.each(caseEntries.map(entry => [entry.name, entry]))("%s", (_name, entry) => {
        ...
        if (rejectedByReference) {
          if (!referenceRejected.has(rejectionKey)) {
            throw new Error(`${rejectionKey} unexpectedly failed in the Babel reference:\n${diff}`);
          }
          if (update) writeExpectation(mode, entry.id, "");
          if (report) {
            reportRows.push({
              mode,
              name: entry.name,
        ...
        if (update) {
          writeExpectation(mode, entry.id, diff);
          return;
        }
        const expected = readExpectation(mode, entry.id);
        if (diff === expected) return;
        const relative = path.relative(
          path.resolve(__dirname, "../.."),
          expectationPath(mode, entry.id)
        );
        if (expected === "") {
          throw new Error(
            `${mode}/${entry.name} was at parity but now diverges. ` +
              `Regenerate intentionally with UPDATE_PARITY=1 and review ${relative}.\n\n${diff}`
          );
        }
        if (diff === "") {
          throw new Error(
            `${mode}/${entry.name} reached parity. Regenerate expectations to delete ${relative}.`
          );
        }
        throw new Error(
          `${mode}/${entry.name} changed from its recorded divergence. ` +
            `Review and regenerate ${relative} if intentional.\n\n${unifiedDiff(expected, diff)}`
        );
      });
    });
  }

  test("has no stale probe expectations", () => {
    if (report) return;
    if (!fs.existsSync(expectedDir)) return;
    const known = new Set();
    for (const mode of Object.keys(modes)) {
      for (const entry of caseEntries) known.add(path.join(mode, `${entry.id}.diff`));
    }
    const stale = [];
  ...
  afterAll(() => {
    ...
    if (process.env.PARITY_REPORT_FILE) {
      fs.writeFileSync(
        process.env.PARITY_REPORT_FILE,
        JSON.stringify({ summary, differing }, null, 2)
      );
    }
  });
});
