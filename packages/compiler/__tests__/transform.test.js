...
const babelDomFixtures = path.resolve(
  __dirname,
...
describe("jsx-dom-expressions-compiler AST-native milestone", () => {
  ...
  it("pre-evaluates component call refs for the Solid 1.x callback fallback", () => {
    ...
    expect(result.code).toContain('typeof _ref$ === "function" && _ref$(r$)');
  });
  ...
  it("supports disabling inline style template serialization", () => {
    ...
    expect(result.code).toContain("_$style(_el$, (() => `color: red`)(), _$p)");
  });
  ...
  it("captures owner context for custom elements when opted in", () => {
    ...
    expect(result.code).toContain("_$template(`<my-element>`, true, false, false)");
  ...
  it("emits Solid 1.x hydratable SSR child slots and eager siblings", () => {
    const result = transform(
      `
    ...
    expect(result.code).toContain(
      "return _$ssr(_tmpl$, _$ssrHydrationKey(), _$escape(props.children), _$escape(_$createComponent(OrderedSibling, {})));"
    );
    expect(result.code).not.toContain("_$scope");
  });
  ...
  it("lowers dynamic children in SSR mode through escape", () => {
    ...
    expect(result.code).toContain("_$ssr(_tmpl$, _$escape(name))");
  });
  ...
  it("lowers DOM spread attributes through spread and mergeProps", () => {
    ...
    expect(result.code).toContain("_$template(`<div id=main>`)");
  ...
  it("lowers plain dynamic DOM attributes through effect and setAttribute", () => {
    ...
    expect(result.code).toContain('_$setAttribute(_el$, "title", title())');
  });
  ...
  it("registers delegated events for member expression handlers", () => {
    ...
    expect(result.code).toContain('import { delegateEvents as _$delegateEvents } from "r-dom";');
    expect(result.code).toContain(
      '_$addEventListener(_el$, "click", counter.increment, true);'
    );
    expect(result.code).toContain('_$delegateEvents(["click"]);');
  });
  ...
  it("treats namespaced event attributes like Babel after the event update", () => {
    ...
    expect(result.code).toContain('_$addEventListener(_el$, "click",');
    expect(result.code).not.toContain("_$delegateEvents");
  });
  ...
  it("lowers unknown namespaced DOM attributes through setAttribute", () => {
    ...
    expect(result.code).toContain('_$setAttribute(_el$, "foo:bar", value())');
  });
  ...
  it("lowers DOM refs with runtime ref helper or assignment fallback", () => {
    ...
    expect(functionRef.code).toContain('import { use as _$use } from "r-dom";');
    expect(functionRef.code).toContain("_$use((el) => register(el), _el$)");
    ...
    expect(identifierRef.code).toContain('typeof _ref$ === "function"');
  ...
  it("lowers call-expression DOM refs without assignment fallback", () => {
    ...
    expect(result.code).toContain("&& _$use(_ref$, _el$)");
  });
  ...
  it("lowers child-property dynamic DOM attributes through effects", () => {
    ...
    expect(result.code).toContain("_el$2.data = label()");
  });
  ...
  it("lowers dynamic style attributes through the style helper", () => {
    ...
    expect(result.code).toContain('import { style as _$style } from "r-dom";');
    expect(result.code).toContain("_$style(_el$, someStyle(), _$p)");
  });
  ...
  it("lowers dynamic style object entries through setStyleProperty", () => {
    ...
    expect(result.code).toContain("_$effect(");
    expect(result.code).toContain('_$setStyleProperty(_el$, "border", signal())');
  });
  ...
  it("passes Solid 1.x class arrays through className", () => {
    const result = transform(
    ...
    expect(result.code).toContain('`static ${{ hi: "k" } || ""}`');
    expect(result.code).toContain("type=button");
    expect(result.code).toContain("_$className");
    expect(result.code).not.toContain("classList.toggle");
  });

  it("passes dynamic Solid 1.x class arrays through className", () => {
    const result = transform('<div class={["todo", { active: isActive() }]} />', {
      filename: "input.jsx",
      moduleName: "r-dom"
    });

    expect(result.code).toContain('`todo ${{ active: isActive() } || ""}`');
    expect(result.code).toContain("_$effect(");
    expect(result.code).toContain("_$className");
  });
  ...
  it("falls back to className helper for arbitrary class array entries", () => {
    ...
    expect(result.code).toContain('_$className(_el$, ["todo", props.active])');
  });
  ...
  it("rejects DOM state dynamic attributes using shared constants", () => {
    ...
    expect(result.code).toContain("_el$.checked = visible()");
  });
