...
const babelPkgDir = path.resolve(compilerDir, "../babel-plugin-jsx-dom-expressions");
...
// Option sets mirror the Solid 1.x suites in
// packages/babel-plugin-jsx-dom-expressions/test/
...
const structuralPlugin = () => ({
  visitor: {
    ...
    ReturnStatement(p) {
      ...
      if (!last || last.type !== "ReturnStatement" || !last.argument) return;
      // A zero-argument arrow IIFE in return position is only a setup block:
      // arrows preserve lexical this/arguments, so flattening its statements
      // into the containing block is semantics-preserving.
      p.replaceWithMultiple([
        ...statements.slice(0, -1),
        t.returnStatement(last.argument)
      ]);
    }
  }
});
...
// Stable comparison result for ratchet suites. Both compilers rejecting (or
...
function compareSource(source, fixture, options) {
  ...
  let babel;
  let babelNormalizeError;
  let oxc;
  let oxcNormalizeError;
  try {
    babel = normalize(babelRaw);
  } catch (error) {
    babelNormalizeError = error;
  }
  try {
    oxc = normalize(oxcRaw);
  } catch (error) {
